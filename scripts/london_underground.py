from dataflows import Flow, load, dump_to_path, PackageWrapper, printer
...
def filter_underground_journeys(rows):
    for row in rows:
        if row['Days in period'] != '':
            yield row

def london_underground_journeys(link):
    Flow(
        load(link, sheet=2, format='xlsx', engine='openpyxl'),
        filter_underground_journeys,

        load(link, sheet=3, format='xlsx', engine='openpyxl'),
        set_format_and_name,
        dump_to_path('data'),
